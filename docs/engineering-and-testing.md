...
| TCU and device component | Timing queue intervals and labels, label broadcast, per-port queue matching, zero intervals, two same-label ports, atomic admission, underflow, missed deadline, resource conflict, clock crossing, fixed codeword-to-output latency, readout and reset. | Exact trigger and physical-output ticks, ordering and typed faults; bounded resource state remains valid. |
