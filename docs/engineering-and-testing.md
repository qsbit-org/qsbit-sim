...
| Unknown, duplicate, consumed, or old-epoch measurement token | Explicit error or specified stale-epoch discard; no overwrite of another generation. |
...
| Different CPU and fast-feedback latencies | Independent delivery and credit release; a later pending measurement does not suppress fast flags. |
