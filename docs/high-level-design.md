...
- `CpuCycleInput`: CPU edge time and cycle, memory response, producer-operation acknowledgment or rejection, CPU-visible measurement result with its token, and reset epoch.
...
- `Feedback`: measurement token, value, status, acquisition-end tick, result-ready tick, and CPU-visible tick.
