...
The ELF loader maps executable segments and entry PC into simulated memory. The ISA library returns a `DecodedOperation` with operand fields, architectural effect, and declared resource usage. The CPU timing model owns register and PC state, pipeline progression, memory and command-port stalls, exception delivery, and retirement. The command adapter converts an issued extension instruction or an M0 MMIO write into the same timing-point and reserved-event protocol; the CPU commits the instruction under its defined admission rule. The quantum pipeline reserves ordered timing labels and fills per-port event queues; the TCU timer broadcasts a reached label to trigger matching device actions on its own clock. The device model handles channel occupancy, readout, and feedback visibility. A device backend performs quantum evolution or returns scripted measurements.
...
- `TimingPoint` and `ReservedEvent`: a timing interval and label, plus a stable command ID, originating instruction ID, port, codeword or payload, target resource, and correlation ID. A derived absolute fire tick may be recorded for checks; it does not replace the timing queue.
...
The producer reserves ordered timing points as interval-and-label entries in a bounded timing queue and places matching events in bounded per-port queues. The deterministic-domain timer counts TCU edges, broadcasts each reached label, and fires all matching port events on that edge. Atomic admission prevents a time point from becoming visible with only part of its same-time event group. The dispatcher checks output-resource conflicts, and the codeword map supplies a fixed trigger-to-output latency. A late reservation, missing event, queue underflow, or resource conflict produces a specified fault rather than an unreported delay. Queue depth, ports and lanes, timer-start convention, codeword map, crossing delay, readout time, and fault policy are configuration values. See [Module Architecture and QuMA-Style Timing Control](module-architecture.md) for each module and its interfaces.
