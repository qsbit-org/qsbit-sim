...
The initial extension profile uses port and codeword commands, waiting, and measurement-result access; message and synchronization instructions are reserved for later profiles. Instruction encodings are not asserted to be binary-compatible with Distributed-HISQ until checked against an authoritative encoding specification. An M0 MMIO doorbell is only another producer of timing-point and reserved-event records, not the Phase 1 software-facing instruction set.

## 5. CPU and control-port contract

All public records use fixed-width values and a versioned serialized form. They include:

- `CpuCycleInput`: CPU edge time and cycle, memory response, producer-operation acknowledgment or rejection, visible measurement token, and reset epoch.
...
Producer acceptance and TCU group admission are separate acknowledgments. APPEND retires after its event and result-slot reservation enter bounded producer staging. Positive ADVANCE and FLUSH seal a point and wait for group admission; READ_RESULT first flushes, then waits for its token; END seals and initiates drain. These are proposed semantic operations, not final encodings. Waiting for TCU admission on every scalar APPEND would prevent later instructions from completing its group. Oversized groups fault immediately. Stable epoch-tagged IDs prevent duplicate acceptance, and only the oldest non-speculative instruction may publish an irreversible side effect. See Section 4 of [the module contract](module-architecture.md) for exact completion and crossing rules.
...
2. **Phase 1 core:** versioned RV32I extension profile, assembler macros, QuMA-style timing and event queues, command backpressure, concurrent output resources, readout and feedback, and a generic trace contract; the separate validation project supplies the CACTUS differential gate.
...
Before implementation, record decisions for the first CPU stage and memory-latency profile, custom instruction encodings, the external CACTUS fixture and probe points, TCU queue and lane capacities, the numerical crossing and device latency profile, and the two backend adapter prototypes. Baseline same-time ordering and operation completion are specified in the module contract. These parameters are proposals until measured or fixed in a dated architecture decision record; the Phase 1 acceptance criterion above is not optional.
