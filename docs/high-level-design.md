...
- `TraceEvent`: schema version, event kind, global tick, epoch, identity, local cycle, port, operation and kind-specific payload; fault records contain a typed error name.
...
The initial CPU, memory, instruction and backend decisions are recorded in ADR 0001. ADR 0002 records the measured limits of reference probes, binary-measurement defects and finite-program termination. These limits must accompany any equivalence claim. Future profiles require their own decisions and regression evidence.
