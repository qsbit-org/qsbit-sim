...
**Transition:** When the timer reaches label L, compare the timing point’s expected IDs and per-port counts with the previously committed queue heads. Only a complete match becomes one candidate launch batch; remove those members once. Ports absent from the list stay idle and future-label entries remain queued. The configured per-port width cannot be bypassed with zero-time loops.
